...
if (MST_Other.Riyou[item_id]) {
	for (var i = 0,list = MST_Other.Riyou[item_id].split(","),m = list.length,txt = ""; i < m; i++) {
		...
		case "7": //他
			...
			break;
		default:
			txt += list[i].substring(1) + "Available<br>";
			break;
		}
	}
...
if (MST_Quest.Riyou[item_id]) { //クエスト
	for (var i = 0,list = MST_Quest.Riyou[item_id].split(","),m = list.length,txt = ""; i < m; i++) {
		...
		txt += "<a href=\"../quest/" + w[0] + ".htm#l" + list[i].substring(0,4) + "\"" + (w[1].charAt(4) ? " class="+w[1].charAt(4) : "") + ">" + MST_Quest.Btype[parseInt(w[1].substring(0,2),16)] + Quest_Season[w[1].charAt(2)] + Quest_Time[w[1].charAt(3)] + ":" + w[3] + MST_Quest.Qtype[parseInt(w[2],16)] + w[4] + "</a> -" + list[i].substring(5) + "x " +(list[i].charAt(4) === "N" ? "Delivery<br>" : "Consumed<br>");
	}
